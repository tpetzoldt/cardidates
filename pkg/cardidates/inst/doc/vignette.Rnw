...
\author{Thomas Petzoldt, René Sachse and Susanne Rolinski}
\title{Quickstart Manual for Package \pkg{cardidates}}
%\VignetteIndexEntry{cardidates Quickstart Manual}



\Abstract{ The \pkg{cardidates} package provides methods for fitting
  four resp. six parametric Weibull functions to environmental
...
version \Sexpr{installed.packages()["cardidates", 3]}, November 19,
2007.
...
means that it is not necessarily important to get the ``true'' values
(what is true?) for one particular time series but to get reproducible
results independently of the person who performs the analysis.

The proposed methods were developed within the AQUASHIFT research
...
Weibull-functions (Method B in the article); other functions may
follow.
...
\item the \pkg{cardidates} package, which is available as source code
  and as Windows, Macintosh and Linux ``binary''. Currently we do not
  yet intend to upload \pkg{cardidates} to CRAN, the comprehensive
  \proglang{R} archive network, however, it is available from the
  \proglang{R} package development server
  R-Forge\footnote{\url{http://r-forge.r-project.org/projects/cardidates/}}
  of one of the authors to obtain the most recent version.
\item \pkg{cardidates} requires two contributed packages named
  \pkg{boot} and \pkg{pastecs} \citep{boot-package,pastecs-package}
  which can be installed directly from the internet. This can be done
  either using the menu (e.g. on Windows) or with the following
  command:
\begin{Sinput}
...
Following documentation and help examples of the package should be
...
\item Results can be visualized with standard \proglang{R} functions
  or an object-specific \code{plot} method.
\end{enumerate}
...
\textbf{doy} (Julian day of the year), \code{y} (the dependent
variable, e.g. phytoplankton biovolume) and \code{flag} (of type
boolean indicating whether the data point should be included in
analysis). The last column (flag) can be used to mask individual data
points. It may contain either FALSE or 0 (zero) to disable
respectively TRUE or 1 (one) to enable data points. If flag is
...
mass development.  Setting the flag of the respective data manually to
FALSE in the optional column provides another, more individual
...
ever it's wise to check and visualize both, data and results. For
plotting all data sets on one figure you may use:

<<fig=FALSE>>=
plot(as.numeric(carditest$sample) * 365 + carditest$doy, carditest$y, type="b")
@
...
\item Copy your data as usual into your favorite spreadsheet program
  ...
  empty lines above and without measurement units below the \textbf{X,
    Y} header or with three colums \textbf{sample}, \textbf{doy},
...
\code{xstart = 70} is an optional value to indicate the time of ice
out after which the phytoplankton spring mass development is expected
to begin:
...
results. All identified are peaks labelled with consecutive numbers
amd the spring is peak marked with a red line:
